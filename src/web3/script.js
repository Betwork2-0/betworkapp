...
var contractAddress = '0x242d2DD24D3ffcFABE4f27580fA9a835100fdE56'; // FIXME: fill this in with your contract's address/hash
...
async function getUsers() {
	const users = await BlockchainSplitwise.methods.getUsersWithTransactions().call({from:web3.eth.defaultAccount}); 
	return users;
}

// TODO: Get the total amount owed by the user specified by 'user'
async function getTotalOwed(user) {
	const owed = await BlockchainSplitwise.methods.getNetDebtForUser(user).call({from:web3.eth.defaultAccount});
	return parseInt(owed);
}
...
async function add_IOU(creditor, amount) {
	...
	if (path != null) {
		...
		await BlockchainSplitwise.methods.updateCycle(path, min_on_cycle).send({from:web3.eth.defaultAccount, gas:3000000});
	}
	return BlockchainSplitwise.methods.add_IOU(creditor, amount - min_on_cycle).send({from:web3.eth.defaultAccount, gas:3000000});
}
